...
@section('page_title', 'Danh sách Thiết Bị')

@section('content')
<div class="mb-8 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Danh sách bài viết về Gadgets</h1>
        <p class="text-gray-600 text-sm mt-1">Quản lý các sản phẩm thiết bị của bạn</p>
    </div>
    <a href="{{ route('gadgets.create') }}" class="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white font-semibold py-2 px-4 rounded-lg transition duration-300 flex items-center gap-2 hover:shadow-lg">
            ...
            <table class="w-full border-collapse bg-white">
                <thead>
                    <tr class="bg-gradient-to-r from-purple-600 to-purple-700 text-white">
                        ...
                        <th class="px-6 py-4 text-left font-semibold">Tên Thiết Bị</th>
                        <th class="px-6 py-4 text-left font-semibold">Thương Hiệu</th>
                ...
                <tbody>
                    @foreach($gadgets as $index => $gadget)
                        <tr class="border-b border-gray-200 hover:bg-gray-50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4">
                                <p class="font-semibold text-gray-800">{{ $gadget->title ?? 'N/A' }}</p>
                            </td>
                            ...
                            {{-- <td class="px-6 py-4 font-bold text-purple-600">
                                {{ number_format($gadget->price ?? 0, 0, ',', '.') }} ₫
                            </td> --}}
                            <td class="px-6 py-4">
                                <div class="flex gap-2 justify-center">
                                    <a href="{{ route('gadgets.show', $gadget) }}" class="inline-block bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg transition-all duration-200 hover:shadow-md">
                                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M10 12a2 2 0 100-4 2 2 0 000 4z"></path>
                                            <path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd"></path>
                                        </svg>
                                    </a>
                                    <a href="{{ route('gadgets.edit', $gadget) }}" class="inline-block bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-2 px-4 rounded-lg transition-all duration-200 hover:shadow-md">
                                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                        </svg>
                                    </a>
                                    <form action="{{ route('gadgets.destroy', $gadget) }}" method="POST" class="inline-block" onsubmit="return confirm('Bạn chắc chắn muốn xóa?');">
                                        @csrf
                                        @method('DELETE')
